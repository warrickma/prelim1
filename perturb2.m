...
function [p1,p2,p3]=perturb2(A,S,tStart,tEnd,a,b,c,d,e,f)
...
rnap_conc=1150/N_A*10^9/(2.8*10^(-13)*0.3)*a; % rnap concentration in nmol/gDW
ribo_conc=45000/N_A*10^9/(2.8*10^(-13)*0.3)*b; % ribosome concentration in nmol/gDW
e_x=60*60;ke1=e_x*c/L(1); % 60 nt/sec; compute for enlongation rate constant ke 
tau1=ke1/20;% tau is unitless;k_i (initiation rate constant) is from bionumber
ke2=e_x/L(2);
tau2=ke2/20;
ke3=e_x/L(3);
tau3=ke3/20;
K_x=0.24*e;%dissociation constant of RNAP in nmol/gDW
K_L=454.64*f;%dissociation constant of ribosome in nmol/gDW
...
tl1=x(1,t)*ribo_conc/K_L*(16.5*60*d/L(1)*3);
tl2=x(2,t)*ribo_conc/K_L*(16.5*60*d/L(2)*3);
tl3=x(3,t)*ribo_conc/K_L*(16.5*60*d/L(3)*3);
...
for t=tStart:tEnd
f1=10^n/(Kd^n+10^n);
f2=x1(4,t)^n/(1000^n+x1(4,t)^n);
f13=x1(4,t)^n/(1000^n+x1(4,t)^n);
f23=x1(5,t)^10/(100000^10+x1(5,t)^10);
...
tx2=(rnap_conc*ke2/tau2*(p_conc/(p_conc+K_x)))*u2;
tx3=(rnap_conc*ke3/tau3*(p_conc/(p_conc+K_x)))*u3;
tl1=x1(1,t)*ribo_conc/K_L*(16.5*60*d/L(1)*3);
tl2=x1(2,t)*ribo_conc/K_L*(16.5*60*d/L(2)*3);
tl3=x1(3,t)*ribo_conc/K_L*(16.5*60*d/L(3)*3);
r=[tx1;tx2;tx3;tl1;tl2;tl3];
x1(:,t+1)=expm(A*1)*x1(:,t)+inv(A)*[expm(A*1)-eye(6)]*S*r;
end
p1=x1(4,50:100);
p2=x1(5,50:100);
p3=x1(6,50:100);
